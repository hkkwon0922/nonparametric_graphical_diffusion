...
"""
Calculates Clenshaw-Curtis weights and steps for numerical integration.
Expands tensors x0, xT, and h to cover multiple time steps.
Computes the intermediate steps X_steps for integrating x0 to xT.
Calls the integrand function to compute the integrand values for each step.
Sums up the weighted integrand values to estimate the integral using Clenshaw-Curtis weights.
Returns the estimated integral for forward mode.
"""
